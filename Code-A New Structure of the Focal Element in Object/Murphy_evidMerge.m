function [ merge_evid ] = Murphy_evidMerge( evid1, evid2 )
...
K = ds_conflictCoeff(tmp_evid, tmp_evid);
% compute the coressponding BPA 
tmp_norm = 0;
for n = 1:len
    merge_fe(n).bpa = 1/(1-K+eps)*mergeBPA(tmp_evid, tmp_evid, merge_fe(n).basicElement);
    tmp_norm = tmp_norm + merge_fe(n).bpa;
end
for n = 1:len
    merge_fe(n).bpa = merge_fe(n).bpa/(tmp_norm + eps);
end
